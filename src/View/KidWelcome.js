...
export default function KidWelcome(props) {
  ...
  const renderItem = ({item}) => {
    return (
      <Pressable
        onPress={() => {
          setCurrentKid(item?.kidData);
          console.log(currentKid, 'post selection');
          console.log(item.kidData, 'selected obj');
        }}>
        <View
          style={[
            checkSelected(item?.kidData?._id) && styles.selected,
            styles.listWhiteBg,
          ]}>
          <Image
  ...
  const handleGetUserData = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        console.log(
          JSON.stringify(res?.data?.familyObj?.kidIdArr, null, 2),
          'family obj',
        );
        ...
        <View style={{paddingHorizontal: Spacing.MARGIN_20}}>
          ...
          <View style={{marginTop: hp(34)}}>
            ...
            <TouchableOpacity
              onPress={() =>
                currentKid
                  ? props.navigation.navigate('KidPin')
                  : (setToggleModal(true), setMessage('Please Choose your Kid'))
              }
              style={{marginTop: Spacing.MARGIN_40}}>
              <LinearGradient
                start={{x: 1, y: 0}}
                end={{x: 0, y: 2}}
                colors={[Colors.GRADIENT1, Colors.GRADIENT2, Colors.GRADIENT1]}
                style={[commonStyle.linearBtn, {borderRadius: 25}]}>
